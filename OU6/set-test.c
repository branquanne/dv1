 ...
 * File:         set-test.c
 *
 * Description:  This file contains a test program for the set module. The test
 * program tests the functions in the set module. The test program tests the
 * creation of sets, insertion of elements into the sets, and tests set
 * operations such as union, intersection, and difference. The test program also
 * tests the functions that check if the set is empty, if the value is a member
 * of the set, if the sets are equal, if the first set is a subset of the second
 * set, and if the set is empty. Lastly, the test program tests the functions
 * that choose a value from the set, remove an element from the set, and get the
 * size of the set. The test program prints OK if the functions work as
 * expected, otherwise FAIL.
 *
...
void check_set_insert() {
  ...
  // Add 100 elements to the set (The helper function populate_set is using
  // set_insert)
...
/*
 * Description: Populate the set with elements from start to end but not
