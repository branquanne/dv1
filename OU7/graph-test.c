...
void depth_first(int node, Graph *graph, bool *visited) {
  ...
  for (int i = 0; i < neighbour_count; i++) {
    // Check if the neighbour is a member of the neighbour set and if it has not
    // been visited
    if (set_member_of(neighbour_nodes[i], neighbour_set->nodes) &&
        !visited[neighbour_nodes[i]]) {
      printf(" -> ");
